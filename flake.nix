      ...
      in {
        ...
        devShells.wezterm-and-nvim = let
          godot_nvim = pkgs.writeShellScript "godot_nvim" ''
            ...
            start_server() {
              "$(${pkgs.which}/bin/which wezterm)" -e $(${pkgs.which}/bin/which nvim) --listen "$server_path"
            }

            open_file_in_remote_nvim() {
              # escape characters that nvim won't
              filename=$(printf %q "$1")
              "$(${pkgs.which}/bin/which wezterm)" -e $(${pkgs.which}/bin/which nvim) --server "$server_path" --remote-send "<C-\><C-n>:n $filename<CR>:call cursor($2)<CR>"
            }
