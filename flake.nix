  ...
  outputs = { nixpkgs, flake-utils, ... }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        build_nvim_script = term_cmd: # bash
          ''
            #!${pkgs.bash}/bin/bash
            ...
            start_server() {
              ${term_cmd} $(which nvim) --listen "$server_path"
            }

            open_file_in_remote_nvim() {
              # escape characters that nvim won't
              filename=$(printf %q "$1")
              ${term_cmd} $(which nvim) --server "$server_path" --remote-send "<C-\><C-n>:n $filename<CR>:call cursor($2)<CR>"
            }
      ...
      in {
        ...
        devShells.wezterm-and-nvim = let
          godot_nvim =
            pkgs.writeShellScript "godot_nvim" (build_nvim_script "wezterm -e");
        in pkgs.mkShell {
